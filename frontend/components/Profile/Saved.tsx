...
import { appendSavedPosts, setSavedPosts } from "@/store/postSlice";
...
const Saved = ({ userProfile, isOwnProfile }: SavedProps) => {
  ...
  useEffect(() => {
    ...
    const getSavedPosts = async () => {
      const getSavedPostsReq = async () =>
        await axios.get(
          `${API_URL_POST}/saved/${userProfile._id}?page=1&limit=12`,
          {
      ...
      if (result) {
        dispatch(setSavedPosts(result.data.data.posts));
  ...
  const loadMorePosts = useCallback(async () => {
    ...
    try {
      const getSavedPostsReq = async () =>
        await axios.get(
          `${API_URL_POST}/saved/${userProfile._id}?page=${page}&limit=12`,
          {
            withCredentials: true,
          }
        );
      const result = await handleAuthRequest(null, getSavedPostsReq);
      if (result && result.data.data.posts.length > 0) {
        // Append new posts to existing ones
        dispatch(appendSavedPosts(result.data.data.posts));
        setHasMore(result.data.data.pagination?.hasMore || false);
        setPage((prev) => prev + 1);
      } else {
        setHasMore(false);
  ...
  if (posts.length < 1) {
    return <NoPostsFound isOwnProfile={isOwnProfile} postType="saved" />;
  }

  return (
    <div className="mt-10">
      {/* Saved post count display */}
      <div className="mb-4">
        <p className="text-gray-600 text-sm">
          {totalPosts === 1 ? "1 saved post" : `${totalPosts} saved posts`}
        </p>
      </div>
      ...
      {!hasMore && posts.length > 12 && (
        <div className="flex justify-center items-center py-8">
          <p className="text-gray-500">
            You&apos;ve reached the end of all saved posts
          </p>
        </div>
      )}
    </div>
